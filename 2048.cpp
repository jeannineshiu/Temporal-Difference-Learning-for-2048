...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				board state_after = move->after_state();
				float sum = 0;
				int ct = 0;
				for (int i = 0; i < 16; ++i) {
					if (state_after.at(i) == 0) {
						++ct;
						// pop up 2
						state_after.set(i, 1);
						sum += 0.9 * estimate(state_after);
						// pop up 4
						state_after.set(i, 2);
						sum += 0.1 * estimate(state_after);
						// clear
						state_after.set(i, 0);
					}
				}
				move->set_value(move->reward() + sum / ct);
...
int main(int argc, const char* argv[]) {
	...
	//std::string csv_filename = "mean_400k.csv";
	...
	tdl.load("model_500k.bin");


	// std::fstream meanlog;
	// meanlog.open(csv_filename, std::ios::out);
	...
	for (size_t n = 1; n <= total; n++) {
		...
		// if(n % 1000 == 0) 
		// 	meanlog << n << "," << mean << std::endl;
	}

	// store the model into file
	// tdl.save("model_400k.bin");
	// meanlog.close();
